...
// function prototypes
int get_aft_age(double** timein,double** tempin,const int m,const int n,double* aftAgesArray);
...
int calc_age(double* t_length,int nsubstp,double* subtime,double* age);
...
int get_aft_age(double** timein,double** tempin,const int m,const int n,double* aftAgesArray)
{
  ...
  deltime = timein[m - 1][0];
  for(i = 0;i < m;i++)
  {
    timein[i][0] = deltime - timein[i][0];
  }
  ...
  {
    time_pad[i] = timein[0][0] + MILYEARS - (double)(i + 1) * time_int;
  }
  
  for(i = 0;i < m;i++)
  {
    time_pad[i + NSTEPS - 1] = timein[i][0];
  }
  ...
  for(int currSample = 0;currSample < n;currSample++)
  {
    ...
    for(i = 0;i < (NSTEPS - 1);i++)
    {
      temp_pad[i] = tempin[0][currSample];
    }
  
    for(i = 0;i < m;i++)
    {
      temp_pad[i + NSTEPS - 1] = tempin[i][currSample];
    }

    // pass temp history to annealing function
    aft_age_fun(time_pad,temp_pad,(m + NSTEPS - 1),&age);

    aftAgesArray[currSample] = age;
...
int anneal(double* subtemp,double* subtime,int nsubstp,double* age2)
{
  ...
  calc_age(t_length,nsubstp,subtime,&age);
  *age2 = age;	
