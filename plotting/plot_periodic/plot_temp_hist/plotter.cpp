...
void Plotter::refreshPixmap(int value)
{
    ...
    update();
    if(value == 1)
    	pixmap.save("../output/plots/image_2D_temp_hist.png", "PNG");
    else if(value == 2)
    {
...
void Plotter::drawGrid(QPainter *painter)
{
  ...
  painter->drawRect(rect);
	...
	dark.setWidth(3);
	painter->setPen(dark);
	painter->drawText(Margin, Margin - 30, width() - 2 * Margin, 20, AlignHCenter | AlignTop, "COOLING HISTORIES: 2D PERIODIC, EROSION RATE = " + exhumation + " Million yrs");
	painter->drawText(Margin, Margin + height() - 65, width() - 2 * Margin, 20, AlignHCenter | AlignBottom, "Time (million yrs)");

	matrix.rotate(-90);
	painter->setWorldMatrix(matrix);
	painter->drawText(Margin-400, Margin - 43, height() - 2 * Margin, 20, AlignLeft | AlignBottom, "Temperature (Celcius)");
	matrix.rotate(90);
	painter->setWorldMatrix(matrix);
}
...
void Plotter::drawCurves(QPainter *painter)
{
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 15, iso_1);
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 30, iso_2);
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 45, iso_3);
}
