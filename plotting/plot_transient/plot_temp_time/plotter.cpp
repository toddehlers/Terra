...
void Plotter::refreshPixmap(int value)
{
    ...
    update();
    if(value == 1)
    	pixmap.save("./output/plots/image_temp_time.png", "PNG");
    else if(value == 2)
    {
...
void Plotter::drawGrid(QPainter *painter)
{
	...
	QRect rect_legend(width() - 140, Margin + 430, 110, 80);
    PlotSettings settings = zoomStack[curZoom];
    QPen quiteLight = colorGroup().light().dark();
    //QPen dark = colorGroup().dark();
	QPen dark(black);
	quiteLight.setStyle(DotLine);
	//quiteLigt.setWidth(5);
    for (int i = 0; i <= settings.numXTicks; ++i) {
        int x = rect.left() + (i * (rect.width() - 1)
                                 / settings.numXTicks);
        double label = settings.minX + (i * settings.spanX()
	...
	//////////////////////////////////////////////////////////////
	/*int x = rect.left() + (3 * (rect.width() - 1)
                                 / settings.numXTicks);*/
	//painter->setPen(dark);
	/*painter->drawText(x - 50, rect.bottom() + 20, 200, 15,
							AlignHCenter | AlignTop,
							"Distance Across Topography (m)");
	painter->drawText((width())/3, rect.bottom() + 20, 200, 15,
							AlignHCenter | AlignTop,
							"Distance Across Topography (m)");*/
	...
	dark.setWidth(3);
	painter->setPen(dark);
	painter->drawText(Margin, Margin - 30, width() - 2 * Margin, 20, AlignHCenter | AlignTop, "CLOSURE TEMPERATURE DEPTH: 1D TRANSIENT EROSION, EROSION RATE = " + exhumation + " mm/yr");
	painter->drawText(Margin, Margin + height() - 65, width() - 2 * Margin, 20, AlignHCenter | AlignBottom, "Time (Million yrs)");
...
void Plotter::drawCurves(QPainter *painter)
{
	...
	painter->fillRect(width() - 139, Margin + 431, 108, 78, brush);
	x1 = width() - 139;
	y1 = Margin + 431;
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 15, iso_1 + " C");
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 30, iso_2 + " C");
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 45, iso_3 + " C");
	...
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 60, iso_4 + " C");
	...
	pen.setColor(red);
	painter->setPen(pen);
	painter->drawLine(x1 + 5, y1 + 70, x1 + 35, y1 + 70);
	pen.setColor(black);
	painter->setPen(pen);
	painter->drawText(x1 + 45, y1 + 75, iso_4);*/

	//pic.save("image.bmp", "BMP");
}
